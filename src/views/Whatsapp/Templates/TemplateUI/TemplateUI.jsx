...
export default function TemplateUI() {
   ...
   const getCurrentTemplate = (templateId, modal) => {
      ...
      setLoader(true)
      // fetch('https://ae49-2402-e280-3d9c-20d-e6f6-f0ed-542e-baa8.ngrok-free.app/getTemplateById/', {
      //    method: 'POST',
      //    body: formData
      // })
      //    .then(response => {
                        ...
                        <CardBody>
                           ...
                           <Row className='match-height'>
                              ...
                              }
                              {
                                 ...
                                 AllTemplatesData && AllTemplatesData.map((SigleTemplate) => {
                                    return (
                                             ...
                                             <div className='mt-auto  '>
                                                <div className='mt-3  rounded-3 d-flex justify-content-evenly  '>
                                                   <div className='d-flex justify-content-evenly position-absolute top-0 end-0' style={{ marginTop: "8px", marginRight: "8" }}>
                                                      ...
                                                      {
                                                         SigleTemplate.status === "APPROVED" && <div className=' border-0 px-1 bg-success text-white rounded-2 shadow-lg '>Approved</div>
                                                      }
                                                      {
                                                         SigleTemplate.status === "REJECTED" && <div className=' border-0 px-1 bg-danger text-white rounded-2'>Rejected</div>
                                                      }
                                                      {
                                                         SigleTemplate.status === "PENDING" && <div className=' border-0 px-1 bg-warning text-white rounded-2'>Pending</div>
                                                      }
                                                   </div>

                                                   <button className='btn btn-primary' onClick={() => nagivate(`/merchant/whatsapp/editTemplate/${SigleTemplate.id}`)} >Edit</button>
                                                   {/* <button className='btn btn-primary' onClick={() => delTemplate(SigleTemplate.name)} >Delete</button> */}
                                                   <button className='btn btn-danger' onClick={() => inactiveTemplate(SigleTemplate.id)}>Activate</button>
                                                   <button className='btn btn-primary' onClick={() => getCurrentTemplate(SigleTemplate.id, 'modal')}>Test</button>
                                                   <button className='btn btn-primary ' onClick={() => getCurrentTemplate(SigleTemplate.id, 'modal2')} >Send to</button>
                                                </div>
                                             </div>
                                          </Card>
                                       </Col>
